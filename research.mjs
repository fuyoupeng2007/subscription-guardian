...
export async function research(q) {
  ...
  out.platforms.push(makeEntry('网易云音乐', netease));
  out.platforms.push(makeEntry('QQ音乐', qq));
  out.platforms.push(makeEntry('B站', bili));
  return out;
}

function makeEntry(name, res) {
  if (!res || res.error) return { platform: name, reachable: false, error: res?.error || '无响应', found: 0, top: [] };
  return { platform: name, reachable: true, found: res.length, top: res.slice(0, 3) };
}

// —— 展示成中文文本（给 REPL / --research / 工具回调用）——
export function renderResearch(r) {
  const L = [];
  L.push(`🔎 真实联网调研「${r.query}」：`);
  for (const p of r.platforms) {
    if (!p.reachable) { L.push(`  • ${p.platform}：⚠️ 查询失败（${p.error}）`); continue; }
    const line = p.found > 0
      ? `  • ${p.platform}：✅ 能搜到（${p.found} 条）；例如「${(p.top[0]?.title || '')} - ${p.top[0]?.artist || p.top[0]?.author || ''}」`
      : `  • ${p.platform}：❌ 未搜到`;
    L.push(line);
  }
  return L.join('\n');
